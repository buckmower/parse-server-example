...
exports.index = function(req, res) {
  ...
  // Display the user profile if user is logged in.
    if (Parse.User.current()) {
        ...
        if((typeof user.id == "undefined") || (user.id == "")) {
          ...
          Parse.Promise.when([
            ...
            Parse.Promise.when([   //r# stands for result# (inputs into .then)
              ...
              Parse.Cloud.run("membersubscriptions", {"pUserID": pUserID}) //r10
              ...
              var userprofileimage = r3 || r9[0]['thumbnailurl'] || null;
              ...
              var membersubscriptions = r10 || false;
     ...
     res.redirect('/home?er=incorrectlogin');
   });
  } else {
...
// Display all posts.
exports.striperequest = function(req, res) {
  ...
  // Display the user profile if user is logged in.
    if (Parse.User.current()) {
      ...
      Parse.User.current().fetch().then(function(user) {
        var name = user.get('name');
        var pUserID = user.id;
        var tokenid = req.body.tokenid || false;
        if(tokenid !== false){
          stripe.initialize("sk_live_9LflrLEwRkkyeFTXFHy4G443");
          stripe.Charges.create({
            amount: 957,
            currency: "usd",
            source: tokenid, // obtained with Stripe.js
            description: "1 yr subscription to "+req.body.spherename
          },{
            success: function(httpResponse) {
              var subscriptionObjects = Parse.Object.extend("SphereSubscriptions");
              var subscriptionsquery = new Parse.Query(subscriptionObjects);
              subscriptionsquery.equalTo("pUserID", pUserID);
              subscriptionsquery.equalTo("sphereObjectID", req.body.sphereobjectid);
              subscriptionsquery.first().then(function(subscriptionobject){
                var nowdatetime = new Date();
                subscriptionobject.set("stripechargeobject", httpResponse);
                subscriptionobject.set("mostRecentPayment", nowdatetime);
                subscriptionobject.save().then(function(){
                  res.redirect("/subscriptions");
                });
              });
            },
            error: function(httpResponse) {
              res.redirect("/subscriptions?er=error");
            }
          });
        } else {
          res.redirect("/subscriptions?er=error2");
        }
      }, function(error){
       res.redirect('/home?er=incorrectlogin');
     });
